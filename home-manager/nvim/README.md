...
| やりたいこと | 操作 |
|-------------|------|
| 型情報をサッと見る | カーソルを合わせて `K`。ユニオン型やジェネリクスも展開表示される |
| ホバー内をスクロール | `K` を2回押すとフローティングウィンドウ内に入れる。長い型定義もスクロールで確認可能 |
| 型の定義元に飛ぶ | `gd` で定義ジャンプ。エイリアスの中身を深堀りしたい時に使う |
| 戻る | `<C-o>` でジャンプ前の位置に戻る |
| 参照箇所を探す | `gr` で型や関数がどこで使われているか一覧表示 |
