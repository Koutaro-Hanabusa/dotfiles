...
> ```
> echo '{"timestamp":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","tool_name":"Skill","skill":"nix-dotfiles"}' >> ~/.config/Codex-otel-monitoring/logs/Codex-hooks.log
> ```
...
**重要**: `mkOutOfStoreSymlink` で管理されているファイル（nvim, Codex, karabiner 等）は、
