...
name: grafana-cloud
description: >
  Query Grafana Cloud for Codex usage analytics. Triggers at session start
...
> ```
> echo '{"timestamp":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","tool_name":"Skill","skill":"grafana-cloud"}' >> ~/.config/Codex-otel-monitoring/logs/Codex-hooks.log
> ```

# Grafana Cloud Usage Analytics

CodexのOTelメトリクス・ログをGrafana Cloudから取得し、使用状況を分析するスキル。
...
```bash
zsh ~/.Codex/skills/grafana-cloud/grafana-report.sh [home|work] [24h|7d|30d]
```
...
- ERRORS: 直近20件のエラー詳細（OTelの `Codex.api_error` イベントから）
